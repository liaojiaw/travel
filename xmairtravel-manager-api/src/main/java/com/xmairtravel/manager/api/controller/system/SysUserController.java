...
import com.xmairtravel.core.server.manager.entity.bo.SysRole;
import com.xmairtravel.core.server.manager.entity.bo.SysUser;
import com.xmairtravel.core.server.manager.service.ISysPostService;
import com.xmairtravel.core.server.manager.service.ISysRoleService;
...
import com.xmairtravel.core.utils.poi.ExcelUtil;
...
public class SysUserController extends BaseController
{
    ...
    public ResultBean importData(MultipartFile file, boolean updateSupport) throws Exception
    {
        ExcelUtil<SysUser> util = new ExcelUtil<SysUser>(SysUser.class);
    ...
    public ResultBean getInfo(@PathVariable(value = "userId", required = false) Long userId)
    {
        ...
        if (StringUtils.isNotNull(userId))
        {
            ...
            ajax.put(ResultBean.DATA_TAG, sysUser);
    ...
    public ResultBean add(@Validated @RequestBody SysUser user)
    {
        ...
        {
            return ResultBean.error("新增用户'" + user.getUserName() + "'失败，登录账号已存在");
        }
        else if (StringUtils.isNotEmpty(user.getPhonenumber())
                && UserConstants.NOT_UNIQUE.equals(userService.checkPhoneUnique(user)))
        {
            return ResultBean.error("新增用户'" + user.getUserName() + "'失败，手机号码已存在");
        }
        else if (StringUtils.isNotEmpty(user.getEmail())
                && UserConstants.NOT_UNIQUE.equals(userService.checkEmailUnique(user)))
        {
            return ResultBean.error("新增用户'" + user.getUserName() + "'失败，邮箱账号已存在");
        }
    ...
    @Log(title = "用户管理", businessType = BusinessType.UPDATE)
    @PutMapping("/resetPwd")
    public ResultBean resetPwd(@RequestBody SysUser user)
    {
        userService.checkUserAllowed(user);
        userService.checkUserDataScope(user.getUserId());
        user.setPassword(SecurityUtil.encryptPassword(user.getPassword()));
    ...
    public ResultBean authRole(@PathVariable("userId") Long userId)
    {
        ResultBean ajax = ResultBean.success();
    ...
    @PutMapping("/authRole")
    public ResultBean insertAuthRole(Long userId, Long[] roleIds)
    {
        userService.checkUserDataScope(userId);
